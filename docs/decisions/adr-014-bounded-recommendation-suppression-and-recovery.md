...
empty v2 set triggers automatic progressive recovery instead of being restored
as a permanent terminal result.
...
The actions navigate directly to the relevant existing surfaces. The exhausted
state does not show `Give me three more` or `Retry`. Retry remains reserved for
transient failure.
...
relevant input or suppression epoch changes.
